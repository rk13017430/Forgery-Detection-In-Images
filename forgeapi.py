...
def detect_compression(input):
    ...
    for idx,ax in enumerate(a1):
        ...
        val,key = np.histogram(data, bins=np.arange(data.min(), data.max()+1),normed = False)
...
def detect_noise(input, blockSize=32):
    ...
    if abs(center1 - center2) > .36: return False
    else: return True
...
def detect_f():
    ...
    # print(res)
    return {"noise":res1,"compression":res2}
